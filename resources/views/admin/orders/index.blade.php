            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse" style="min-width:640px">
                    ...
                    <tbody class="divide-y divide-gray-300">
                                ...
                                <tr class="hover:bg-white/40 transition-colors">
                                    <td class="py-5 px-4">
                                        <div class="flex items-center gap-3">
                                            <img src="{{ asset($item->product->image ?? 'images/placeholder-product.png') }}"
                                                class="w-12 h-12 rounded object-cover">
                                            <span
                                                class="font-semibold text-gray-800">{{ $item->product->name ?? 'Deleted Product' }}</span>
                                        </div>
                                    </td>
                                    <td class="py-5 px-4">
                                    ...
                                    <td class="py-5 px-4">
                                        @if($item->product)
                                            <span
                                                class="font-bold {{ $item->product->stock > 0 ? 'text-gray-900' : 'text-red-600' }}">{{ $item->product->stock }}</span>
                                        @else
                                            <span class="text-gray-400">-</span>
                                        @endif
                                    </td>

                                    {{-- Action buttons rendered once per order --}}
                                    @if($loop->first)
                                        <td class="py-5 px-4 flex justify-end gap-2 flex-col space-y-2 items-end"
                                            rowspan="{{ count($order->orderItems) }}">
                                            <form action="{{ route('admin.orders.reject', $order->id) }}" method="POST"
                                                onsubmit="return confirm('Tolak pesanan ini dan kembalikan stok?');">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="bg-[#FF0000] hover:bg-red-600 text-white font-bold py-1.5 px-6 rounded-lg text-sm w-28 text-center transition-colors">
                                                    TOLAK
                                                </button>
                                            </form>
                                            <form action="{{ route('admin.orders.accept', $order->id) }}" method="POST">
                                                ...
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="bg-[#4A0505] hover:bg-[#310303] text-white font-bold py-1.5 px-6 rounded-lg text-sm w-28 text-center transition-colors">
                                                    TERIMA
                                                </button>
                                            </form>
                                        </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-8 text-center text-gray-500 font-medium">Belum ada pesanan masuk.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
